...
impl Connection {
    ...
    pub async fn login_with_adapter<A: GridLoginAdapter>(
        ...
        let response = self.http_transport_login(&request).await?;
    ...
    async fn http_transport_login(
        ...
        let response = client.post(&self.config.endpoint).json(request).send().await?;
...
mod tests {
    use super::*;
    use serde_json::json;
    use viewer_grid::{SecondLifeAdapter, StartLocation, StartLocationIntent};
    ...
    async fn adapter_login_success_sets_logged_in_state_and_session() {
        ...
        assert_eq!(session.session_token, "22222222-2222-2222-2222-222222222222");
    }
