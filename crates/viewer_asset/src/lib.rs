...
use viewer_core::{Vertex, VolumeParams, SculptType};
...
impl GeometryCache {
    ...
    pub fn get_procedural(&mut self, params: &VolumeParams, detail: f32) -> Arc<ProcessedMesh> {
        ...
        let mesh = Arc::new(ProcessedMesh { vertices, submeshes, aabb });
        self.procedural.insert(params.clone(), Arc::clone(&mesh));
        mesh
    }

    pub fn get_sculpt(&mut self, uuid: &str, sculpt_type: SculptType, pixels: &[u8], width: u32, height: u32) -> Arc<ProcessedMesh> {
...
pub fn get_lod_level(distance: f32) -> u32 {
    if distance < 15.0 { 0 }      // High
    else if distance < 40.0 { 1 } // Mid
    else if distance < 80.0 { 2 } // Low
    else { 3 }                    // Tiny
}
