use crate::{Aabb, HoleType, PathType, ProfileType, Vertex, VolumeParams};
...
pub fn generate_volume_mesh(
    ...
    let path_closed = matches!(params.path_type, PathType::Circle)
        && (params.revolutions - params.revolutions.round()).abs() < 1.0e-4
        && path.len() > 2;
    let path_ring_count = if path_closed {
        // generate_path includes the endpoint at t=1 which duplicates the start when revolutions is an integer.
    ...
    for (i, pt) in path.iter().enumerate().take(path_len) {
        ...
        for (j, p) in outer_profile.iter().enumerate() {
            ...
            if j < profile_len - 1 {
                ...
                let next_row = next_i * profile_len + j;
    ...
    if !path_closed {
        ...
        for p in &outer_profile {
            ...
            let vertex_pos = [
                rotated_p[0] + begin_pt.pos[0],
                rotated_p[1] + begin_pt.pos[1],
                rotated_p[2] + begin_pt.pos[2],
            ];
            for k in 0..3 {
        ...
        for p in &outer_profile {
            ...
            let vertex_pos = [
                rotated_p[0] + end_pt.pos[0],
                rotated_p[1] + end_pt.pos[1],
                rotated_p[2] + end_pt.pos[2],
            ];
            for k in 0..3 {
                min[k] = min[k].min(vertex_pos[k]);
                max[k] = max[k].max(vertex_pos[k]);
        ...
        for (i, pt) in path.iter().enumerate().take(path_len) {
            ...
            for (j, p) in inner.iter().enumerate() {
                ...
                if j < profile_len - 1 {
                    ...
                    let next_row = base_inner_offset + next_i * profile_len + j;
...
fn generate_profiles(params: &VolumeParams, detail: f32) -> (Vec<[f32; 2]>, Option<Vec<[f32; 2]>>) {
    let outer = match params.profile_type {
        ...
        ProfileType::TriangleIso | ProfileType::TriangleEqual | ProfileType::TriangleRight => {
            gen_ngon(3, 0.0, 1.0, params.begin_cut, params.end_cut)
        }
        ProfileType::HalfCircle => gen_ngon(
            (12.0 * detail) as u32,
            0.5,
            0.5,
            params.begin_cut,
    ...
    let inner = if params.hollow > 0.0 {
        Some(match params.hole_type {
            HoleType::Same => outer
                .iter()
                .map(|p| [p[0] * params.hollow, p[1] * params.hollow])
                .collect(),
            HoleType::Square => {
                gen_ngon(4, -0.375, params.hollow, params.begin_cut, params.end_cut)
            }
            HoleType::Circle => gen_ngon(
                (24.0 * detail) as u32,
                0.0,
                params.hollow,
                params.begin_cut,
...
fn generate_path(params: &VolumeParams, detail: f32) -> Vec<PathPt> {
    ...
    let steps = match params.path_type {
        PathType::Line => 2,
        PathType::Circle => ((24.0 * detail * params.revolutions.abs().max(1.0)) as u32).max(8),
    };
    ...
    for i in 0..steps {
        ...
        match params.path_type {
            ...
            PathType::Circle => {
                ...
                pt.pos = [ang.cos() * radius, 0.0, ang.sin() * radius];
                // Orient local +Z along the circle tangent (active rotation).
                let yaw = -ang;
                let half = 0.5 * yaw;
                pt.rot = [0.0, half.sin(), 0.0, half.cos()];
            }
        }
        ...
        // 3. Taper & Scale
        pt.scale = [1.0 - t * params.taper_x, 1.0 - t * params.taper_y];
