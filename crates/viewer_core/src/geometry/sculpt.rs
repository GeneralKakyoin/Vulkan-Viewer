use crate::{Aabb, SculptType, Vertex};

pub fn generate_sculpt_mesh(
    pixels: &[u8],
    width: u32,
    height: u32,
    ...
    if pixels.is_empty() || width == 0 || height == 0 {
        return (
            vertices,
            indices,
            Aabb::new([0.0, 0.0, 0.0], [0.1, 0.1, 0.1]),
        );
    }

    let channels = if pixels.len() as u32 >= width * height * 4 {
        4
    } else {
        3
    };
