...
fn network_debug_section_default_open(title: &str) -> bool {
    matches!(title, "Session" | "LLUDP" | "RegionObjects")
}

fn network_debug_section_line_cap(title: &str) -> usize {
    if title == "Session" { 8 } else { 12 }
}
                ...
                egui::Window::new("Network Debug")
                    ...
                    .show(ctx, |ui| {
                        ...
                        for section in &network_debug.sections {
                            let cap = network_debug_section_line_cap(&section.title);
                            let tail_from = section.lines.len().saturating_sub(cap);
                            let visible_lines = &section.lines[tail_from..];
                            egui::CollapsingHeader::new(section.title.as_str())
                                .default_open(network_debug_section_default_open(&section.title))
                                .show(ui, |ui| {
                                    if visible_lines.is_empty() {
                                        ui.label("none");
                                    } else {
                                        if section.lines.len() > visible_lines.len() {
                                            ui.small(format!(
                                                "showing last {}/{} lines",
                                                visible_lines.len(),
                                                section.lines.len()
